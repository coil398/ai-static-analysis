...
1. **スキル定義** (`<name>.md`、レポジトリルート) — Claude への指示書。何をするか・どう実行するかを自然言語で記述。
...
レポジトリルートに `<name>.md` を「スキル定義テンプレート」に従って作成する。
...
スキル定義は一度作って終わりではなく、実装が進むにつれて更新が必要になる。

- 実装中に新たな要件や制約が判明した場合
- SPEC.md の変更に追従する必要がある場合
- 他のスキルとの整合性を保つ必要がある場合

このような場合は `improve-skill` を使ってスキル定義を更新する。

---

## スキル定義テンプレート

新しいスキルの `<name>.md`（レポジトリルート）は以下の構造に従う。
...
- スキル定義は一度作って終わりではなく、実装が進むにつれて `improve-skill` で継続的に改善する。
